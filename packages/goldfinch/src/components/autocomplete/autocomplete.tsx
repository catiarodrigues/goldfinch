...
import { inputVariants, GOLDFINCH_INPUT_VARIANTS } from "../input/input";
...
export const GOLDFINCH_AUTOCOMPLETE_VARIANTS = {
  size: GOLDFINCH_INPUT_VARIANTS.size,
} as const;

export const GOLDFINCH_AUTOCOMPLETE_DEFAULT_VARIANTS = {
  size: "base",
} as const;

// Derived types from GOLDFINCH_AUTOCOMPLETE_VARIANTS
export type GoldfinchAutocompleteSize = keyof typeof GOLDFINCH_AUTOCOMPLETE_VARIANTS.size;

export interface GoldfinchAutocompleteVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: GoldfinchAutocompleteSize;
}

export function autocompleteVariants({
  size = GOLDFINCH_AUTOCOMPLETE_DEFAULT_VARIANTS.size,
}: GoldfinchAutocompleteVariantsProps = {}) {
  return cn(
    resolveVariant(
      GOLDFINCH_INPUT_VARIANTS.size,
      size,
      GOLDFINCH_AUTOCOMPLETE_DEFAULT_VARIANTS.size,
    ).classes,
  );
}
        ...
        <AutocompleteBase.Popup
          className={(state: AutocompleteBase.Popup.State) =>
            cn(
              ...
              "bg-goldfinch-control text-goldfinch-default",
...
function Item({ children, ...props }: AutocompleteBase.Item.Props) {
  return (
    <AutocompleteBase.Item
      data-goldfinch-component="Autocomplete"
      data-goldfinch-part="item"
      {...props}
      className="group mx-1.5 grid cursor-pointer grid-cols-[1fr_16px] gap-2 rounded px-2 py-1.5 text-base data-highlighted:bg-goldfinch-overlay data-selected:font-medium"
    >
...
function GroupLabel(props: AutocompleteBase.GroupLabel.Props) {
  return (
    <AutocompleteBase.GroupLabel
      {...props}
      className={cn(
        "mx-1.5 px-2 py-1.5 text-sm text-goldfinch-strong",
        props.className,
      )}
    />
  );
...
function Group(props: AutocompleteBase.Group.Props) {
  return (
    <AutocompleteBase.Group
      {...props}
      className="border-t border-goldfinch-line mt-2 pt-2 first:border-t-0 first:mt-0 first:pt-0"
    />
  );
}
...
function Separator(props: AutocompleteBase.Separator.Props) {
  return (
    <AutocompleteBase.Separator
      {...props}
      className={cn("mx-0 my-1 h-px bg-goldfinch-line", props.className)}
    />
  );
}
