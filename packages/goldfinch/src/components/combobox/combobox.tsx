...
export const GOLDFINCH_COMBOBOX_VARIANTS = {
...
export const GOLDFINCH_COMBOBOX_DEFAULT_VARIANTS = {
  size: "base",
  inputSide: "right",
} as const;
...
// Derived types from GOLDFINCH_COMBOBOX_VARIANTS
export type GoldfinchComboboxSize = keyof typeof GOLDFINCH_COMBOBOX_VARIANTS.size;
export type GoldfinchComboboxInputSide =
  keyof typeof GOLDFINCH_COMBOBOX_VARIANTS.inputSide;

export interface GoldfinchComboboxVariantsProps {
  /**
   ...
   * @default "right"
   */
  inputSide?: GoldfinchComboboxInputSide;
}

export function comboboxVariants({
  inputSide = GOLDFINCH_COMBOBOX_DEFAULT_VARIANTS.inputSide,
}: GoldfinchComboboxVariantsProps = {}) {
  return cn(
    resolveVariant(
      GOLDFINCH_COMBOBOX_VARIANTS.inputSide,
      inputSide,
      GOLDFINCH_COMBOBOX_DEFAULT_VARIANTS.inputSide,
    ).classes,
  );
}

// Legacy type alias for backwards compatibility
export type ComboboxInputSide = GoldfinchComboboxInputSide;
export type ComboboxSize = GoldfinchComboboxSize;
 ...
 * ```
 */
export interface ComboboxProps extends GoldfinchComboboxVariantsProps {
      ...
      >
        <ComboboxBase.Popup
          className={cn(
            ...
            "bg-goldfinch-base text-goldfinch-default", // background
            "rounded-lg shadow-lg ring ring-goldfinch-line", // border part
            className,
          )}
        >
          {children}
...
function TriggerValue({
  ...
  return (
    ...
    >
      ...
      <ComboboxBase.Icon
        className={cn(
          "absolute top-1/2 -translate-y-1/2 flex items-center text-goldfinch-subtle",
  ...
  return (
    ...
    >
      ...
      <ComboboxBase.Trigger
        ...
        className={cn(
          "absolute top-1/2 -translate-y-1/2 flex items-center justify-center cursor-pointer text-goldfinch-subtle",
  ...
  return (
    <ComboboxBase.Item
      ...
      className={cn(
        ...
        "cursor-pointer data-highlighted:bg-goldfinch-tint",
        ...
        "data-[disabled]:cursor-not-allowed data-[disabled]:text-goldfinch-subtle data-[disabled]:opacity-60 data-[disabled]:data-highlighted:bg-transparent",
        className,
      )}
    >
...
function Empty(props: ComboboxBase.Empty.Props) {
  return (
    <ComboboxBase.Empty
      {...props}
      className={cn(
        "mx-1.5 shrink-0 px-4 py-2 text-[0.925rem] leading-4 text-goldfinch-subtle empty:m-0 empty:p-0",
      )}
...
function GroupLabel(props: ComboboxBase.GroupLabel.Props) {
  return (
    <ComboboxBase.GroupLabel
      {...props}
      className={cn(
        "mx-1.5 px-2 py-1.5 text-sm text-goldfinch-subtle",
        props.className,
      )}
    />
  );
...
function Group(props: ComboboxBase.Group.Props) {
  return (
    <ComboboxBase.Group
      {...props}
      className="border-t border-goldfinch-hairline mt-2 pt-2 first:border-t-0 first:mt-0 first:pt-0"
    />
  );
}
  ...
  return (
    <ComboboxBase.Chip
      ...
      className={cn(
        ...
        "rounded-sm ring-1 ring-goldfinch-hairline", // Border
        "bg-goldfinch-overlay", // Background
        "text-sm", // Typography
      )}
    >
      {props.children}
      <ComboboxBase.ChipRemove
        data-goldfinch-component="Combobox"
        data-goldfinch-part="chip-remove"
        aria-label={removeLabel}
        className={cn(
          "cursor-pointer rounded-md p-1 hover:bg-goldfinch-fill-hover",
          "bg-transparent flex",
        )}
      >
        <XIcon size={10} />
...
const sizeToMinHeight: Record<GoldfinchComboboxSize, string> = {
  xs: "min-h-5",
  sm: "min-h-6.5",
  base: "min-h-9",
  lg: "min-h-10",
