...
export const GOLDFINCH_SELECT_VARIANTS = {
  size: GOLDFINCH_INPUT_VARIANTS.size,
} as const;

export const GOLDFINCH_SELECT_DEFAULT_VARIANTS = {
  size: "base",
} as const;
 ...
 */
export const GOLDFINCH_SELECT_STYLING = {
  trigger: {
    height: 36, // h-9
    paddingX: 12, // px-3
...
// Derived types from GOLDFINCH_SELECT_VARIANTS
export type GoldfinchSelectSize = keyof typeof GOLDFINCH_SELECT_VARIANTS.size;

export interface GoldfinchSelectVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: GoldfinchSelectSize;
}

const selectSizeClasses: Record<GoldfinchInputSize, string> = {
  xs: "h-5 px-1.5 text-xs rounded-sm",
  sm: "h-6.5 px-2 text-xs rounded-md",
  base: "h-9 px-3 text-base rounded-lg",
  lg: "h-10 px-4 text-base rounded-lg",
};

export function selectVariants({
  size = GOLDFINCH_SELECT_DEFAULT_VARIANTS.size,
}: GoldfinchSelectVariantsProps = {}) {
  return cn(
    "inline-flex items-center justify-between font-normal",
    "bg-goldfinch-base text-goldfinch-default ring ring-goldfinch-line hover:bg-goldfinch-tint",
    selectSizeClasses[size],
    "focus:opacity-100 focus:ring-goldfinch-focus/50 focus-visible:ring-inset *:in-focus:opacity-100",
  );
}
...
> = {
  xs: { iconSize: 12, className: "text-goldfinch-subtle" },
  sm: { iconSize: 14, className: "text-goldfinch-subtle" },
  base: { iconSize: 16, className: "text-goldfinch-subtle" },
  lg: { iconSize: 18, className: "text-goldfinch-subtle" },
};
 ...
 * Wraps Base UI Select with Goldfinch styling and optional Field integration.
 *
 * @example
 * ```tsx
...
export function Select<T, Multiple extends boolean | undefined = false>({
  ...
  if (process.env.NODE_ENV !== "production" && hideLabel !== undefined) {
    console.warn(
      "[Goldfinch Select]: `hideLabel` is deprecated. For hidden labels, use `aria-label` instead of `label` + `hideLabel={true}`.\n" +
        "  Migration:\n" +
  ...
  const valueChildrenFn = renderValue
    ? (value: unknown) => {
        const placeholderNode =
          placeholder != null ? (
            <span className="text-goldfinch-placeholder">{placeholder}</span>
          ) : null;
  ...
  // Exclude Goldfinch-extended `items` from Base UI spread — we pass `normalizedItems` instead
  ...
  const selectLabelNode = label ? (
    <SelectBase.Label className="m-0 select-none text-base font-medium text-goldfinch-default">
      <Label
  ...
  const selectControl = (
    ...
    >
      {selectLabelNode}
      <SelectBase.Trigger
        data-goldfinch-component="Select"
        data-goldfinch-part="trigger"
        className={cn(
          selectVariants({ size }),
          props.disabled && "cursor-not-allowed opacity-50",
          error &&
            "!ring-goldfinch-danger focus:ring-goldfinch-danger/50 focus:ring-[1.5px]",
          className,
        )}
      ...
      >
        {loading ? (
          ...
          <SelectBase.Value
            ...
            className="min-w-0 truncate data-[placeholder]:text-goldfinch-placeholder"
          >
            {valueChildrenFn}
          </SelectBase.Value>
        )}
      ...
      <SelectBase.Portal container={container}>
        <SelectBase.Positioner>
          <SelectBase.Popup
            className={cn(
              "flex flex-col",
              "max-h-[var(--available-height)] bg-goldfinch-base text-goldfinch-default",
              "rounded-lg shadow-lg ring ring-goldfinch-line",
  ...
  return (
    <div className="grid gap-2">
      ...
      {selectControl}
      {normalizedError ? (
        <span className="text-sm text-goldfinch-danger">
          {normalizedError.message}
        </span>
      ) : (
        description && (
          <span className="text-sm leading-snug text-goldfinch-subtle">
            {description}
          </span>
        )
      )}
...
const GroupLabel = forwardRef<HTMLDivElement, GroupLabelProps>(
  ({ children, className }, ref) => (
    <SelectBase.GroupLabel
      ref={ref}
      className={cn(
        "px-3.5 py-1.5 text-sm font-semibold text-goldfinch-subtle",
        className,
      )}
    >
      {children}
...
const Separator = forwardRef<HTMLDivElement, SeparatorProps>(
  ({ className }, ref) => (
    <SelectBase.Separator
      ref={ref}
      className={cn("-mx-1 my-1 h-px bg-goldfinch-hairline", className)}
    />
  ),
);
