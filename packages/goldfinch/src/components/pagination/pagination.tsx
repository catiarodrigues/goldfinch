...
export const GOLDFINCH_PAGINATION_VARIANTS = {
  controls: {
    full: {
      classes: "",
      description:
...
export type GoldfinchPaginationControls =
  keyof typeof GOLDFINCH_PAGINATION_VARIANTS.controls;

export const GOLDFINCH_PAGINATION_DEFAULT_VARIANTS = {
  controls: "full",
} as const;

export interface GoldfinchPaginationVariantsProps {
  controls?: GoldfinchPaginationControls;
}

export function paginationVariants({
  controls = GOLDFINCH_PAGINATION_DEFAULT_VARIANTS.controls,
}: GoldfinchPaginationVariantsProps = {}) {
  return cn(
    "flex items-center justify-between gap-2",
    resolveVariant(GOLDFINCH_PAGINATION_VARIANTS.controls, controls, GOLDFINCH_PAGINATION_DEFAULT_VARIANTS.controls).classes,
  );
}
...
function PaginationInfo({ children, className }: PaginationInfoProps) {
  ...
  return (
    <div
      ...
      className={cn("text-sm text-goldfinch-subtle", className)}
    >
      {content}
    </div>
  );
...
function PaginationPageSize({
  ...
  return (
    ...
    >
      {label && <span className="text-sm text-goldfinch-subtle">{label}</span>}
      <Select
...
export interface PaginationControlsProps extends GoldfinchPaginationVariantsProps {
  /**
          ...
          {controls === "full" &&
            (pageSelector === "dropdown" ? (
              <Select
                ...
                className="rounded-none ring-goldfinch-hairline"
                value={page}
                onValueChange={(value) => {
...
function PaginationSeparator({ className }: PaginationSeparatorProps) {
  return (
    <div
      ...
      className={cn("mx-2 h-6 border-l border-goldfinch-hairline", className)}
    />
  );
}
...
export interface PaginationLegacyProps
  extends PaginationBaseProps, GoldfinchPaginationVariantsProps {
  children?: never;
...
function PaginationRoot(props: PaginationProps) {
  ...
  const controls =
    "controls" in props
      ? (props.controls ?? GOLDFINCH_PAGINATION_DEFAULT_VARIANTS.controls)
      : GOLDFINCH_PAGINATION_DEFAULT_VARIANTS.controls;
    ...
    <PaginationContext.Provider value={contextValue}>
      ...
      >
        <div
          ...
          aria-atomic="true"
          data-slot="pagination-info"
          className="grow text-sm text-goldfinch-subtle"
        >
          {getPaginationText()}
        </div>
