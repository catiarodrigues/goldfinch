...
export const GOLDFINCH_SWITCH_VARIANTS = {
  size: {
    sm: {
      classes: "h-5.5 w-8.5",
  ...
  variant: {
    default: {
      classes: "",
      description: "Default switch with pill shape",
    },
    neutral: {
      classes: "",
      description: "Monochrome switch with pill shape for subtle toggles",
    },
  },
} as const;

export const GOLDFINCH_SWITCH_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;

// Derived types from GOLDFINCH_SWITCH_VARIANTS
export type GoldfinchSwitchSize = keyof typeof GOLDFINCH_SWITCH_VARIANTS.size;
export type GoldfinchSwitchVariant = keyof typeof GOLDFINCH_SWITCH_VARIANTS.variant;

export interface GoldfinchSwitchVariantsProps {
  /**
   * Switch size.
   ...
   * @default "default"
   */
  variant?: GoldfinchSwitchVariant;
}

export function switchVariants({
  size = GOLDFINCH_SWITCH_DEFAULT_VARIANTS.size,
  variant = GOLDFINCH_SWITCH_DEFAULT_VARIANTS.variant,
}: GoldfinchSwitchVariantsProps = {}) {
  const sizeConfig = resolveVariant(GOLDFINCH_SWITCH_VARIANTS.size, size, GOLDFINCH_SWITCH_DEFAULT_VARIANTS.size);
  const variantConfig = resolveVariant(GOLDFINCH_SWITCH_VARIANTS.variant, variant, GOLDFINCH_SWITCH_DEFAULT_VARIANTS.variant);
  return cn(sizeConfig.classes, variantConfig.classes);
}

// Legacy type aliases for backwards compatibility
export type SwitchSize = GoldfinchSwitchSize;
export type SwitchVariant = GoldfinchSwitchVariant;
 ...
 * ```
 *
 * Neutral variant (monochrome, pill shape):
 * ```tsx
...
export type SwitchProps = Omit<
  ...
  "children"
> & {
  /** Visual variant: "default" (pill, brand accent) or "neutral" (pill, monochrome) */
        ...
        render={(rootProps, state) => {
          ...
          return (
            <button
              ...
              {...props}
              ref={rootRef}
              data-goldfinch-component="Switch"
              type="button"
              role={role}
              {...checkedA11yProps}
