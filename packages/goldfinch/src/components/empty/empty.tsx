...
export const GOLDFINCH_EMPTY_VARIANTS = {
  size: {
    sm: {
...
export const GOLDFINCH_EMPTY_DEFAULT_VARIANTS = {
  size: "base",
} as const;

export type GoldfinchEmptySize = keyof typeof GOLDFINCH_EMPTY_VARIANTS.size;

export interface GoldfinchEmptyVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: GoldfinchEmptySize;
}

export function emptyVariants({
  size = GOLDFINCH_EMPTY_DEFAULT_VARIANTS.size,
}: GoldfinchEmptyVariantsProps = {}) {
  return cn(
    "flex w-full flex-col items-center rounded-xl border border-goldfinch-fill bg-goldfinch-control text-goldfinch-default",
    resolveVariant(GOLDFINCH_EMPTY_VARIANTS.size, size, GOLDFINCH_EMPTY_DEFAULT_VARIANTS.size).classes,
  );
}
 ...
 * ```
 */
export interface EmptyProps extends GoldfinchEmptyVariantsProps {
