...
export const GOLDFINCH_LINK_VARIANTS = {
  variant: {
    inline: {
      classes:
        // text-goldfinch-link provides defensive color that won't be overridden by global `a` styles
        "text-goldfinch-link underline underline-offset-[0.15em] decoration-[0.0625em] link-current transition-colors",
    ...
    plain: {
      classes:
        // text-goldfinch-link provides defensive color that won't be overridden by global `a` styles
        "text-goldfinch-link hover:text-goldfinch-link/70 transition-colors",
      description: "Link without underline decoration",
    },
  },
} as const;

export const GOLDFINCH_LINK_DEFAULT_VARIANTS = {
  variant: "inline",
} as const;

export type GoldfinchLinkVariant = keyof typeof GOLDFINCH_LINK_VARIANTS.variant;

export interface GoldfinchLinkVariantsProps {
  /**
   ...
   * @default "inline"
   */
  variant?: GoldfinchLinkVariant;
}

export function linkVariants({
  variant = GOLDFINCH_LINK_DEFAULT_VARIANTS.variant,
}: GoldfinchLinkVariantsProps = {}) {
  return cn(resolveVariant(GOLDFINCH_LINK_VARIANTS.variant, variant, GOLDFINCH_LINK_DEFAULT_VARIANTS.variant).classes);
}
