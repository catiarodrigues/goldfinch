...
export const GOLDFINCH_TABLE_OF_CONTENTS_VARIANTS = {
  state: {
    default: {
      classes:
        "text-goldfinch-subtle hover:border-goldfinch-line hover:text-goldfinch-default hover:font-medium",
      description: "Inactive section link",
    },
    active: {
      classes: "border-goldfinch-brand font-medium text-goldfinch-default",
      description: "Currently visible / active section",
    },
  },
} as const;

export const GOLDFINCH_TABLE_OF_CONTENTS_DEFAULT_VARIANTS = {
  state: "default",
} as const;

export type GoldfinchTableOfContentsState =
  keyof typeof GOLDFINCH_TABLE_OF_CONTENTS_VARIANTS.state;
...
>(({ className, ...props }, ref) => (
  <p
    ref={ref}
    className={cn(
      "mb-3 text-xs font-semibold tracking-wide text-goldfinch-subtle uppercase",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    className={cn(
      "flex flex-col gap-2 border-l-2 border-goldfinch-hairline",
      className,
    )}
    {...props}
  />
...
>(({ active = false, className, children, render, ...props }, ref) => {
  const stateClasses = active
    ? GOLDFINCH_TABLE_OF_CONTENTS_VARIANTS.state.active.classes
    : GOLDFINCH_TABLE_OF_CONTENTS_VARIANTS.state.default.classes;
  ...
  const sharedProps = {
    ...
    "data-goldfinch-component": "TableOfContents",
...
const NESTED_UL_CLASSES =
  "flex flex-col gap-2 border-l-2 border-goldfinch-hairline [&>li>a]:pl-7 [&>li>button]:pl-7";
...
>(({ label, href, active = false, className, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        <a
          ...
          aria-current={active ? ("true" as const) : undefined}
          data-goldfinch-component="TableOfContents"
          data-goldfinch-part="group-link"
  ...
  return (
    <li
      ...
      {...props}
    >
      <p className="py-0.5 pl-4 text-sm leading-5 font-medium text-goldfinch-subtle">
        {label}
      </p>
      <ul className={cn(NESTED_UL_CLASSES)}>{children}</ul>
    </li>
