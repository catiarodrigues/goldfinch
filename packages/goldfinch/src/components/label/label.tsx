...
export const GOLDFINCH_LABEL_VARIANTS = {
  // Label currently has no variant options but structure is ready for future additions
} as const;

export const GOLDFINCH_LABEL_DEFAULT_VARIANTS = {} as const;

// Derived types from GOLDFINCH_LABEL_VARIANTS
export interface GoldfinchLabelVariantsProps {}

export function labelVariants(_props: GoldfinchLabelVariantsProps = {}) {
  return cn(
    ...
    "m-0 text-base font-medium text-goldfinch-default",
  );
}
 ...
 * ```
 */
export interface LabelProps extends GoldfinchLabelVariantsProps {
...
export function Label({
  ...
  const content = (
    <>
      {children}
      {showOptional && (
        <span className="font-normal text-goldfinch-subtle">(optional)</span>
      )}
      {tooltip && (
        <Tooltip
          content={tooltip}
