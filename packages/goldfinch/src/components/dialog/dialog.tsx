...
export const GOLDFINCH_DIALOG_VARIANTS = {
  size: {
    base: {
      classes: "sm:min-w-96",
...
export const GOLDFINCH_DIALOG_DEFAULT_VARIANTS = {
  size: "base",
  role: "dialog",
} as const;

export const GOLDFINCH_DIALOG_STYLING = {
  dimensions: {
    sm: {
      width: 350,
      titleSize: 20,
...
// Derived types from GOLDFINCH_DIALOG_VARIANTS
export type GoldfinchDialogSize = keyof typeof GOLDFINCH_DIALOG_VARIANTS.size;
export type GoldfinchDialogRole = keyof typeof GOLDFINCH_DIALOG_VARIANTS.role;

export interface GoldfinchDialogVariantsProps {
  /**
   * Dialog width.
   ...
   * @default "base"
   */
  size?: GoldfinchDialogSize;
}

// Dialog Role Context

const DialogRoleContext = createContext<GoldfinchDialogRole>("dialog");
...
export function dialogVariants({
  size = GOLDFINCH_DIALOG_DEFAULT_VARIANTS.size,
}: GoldfinchDialogVariantsProps = {}) {
  return cn(
    // Base styles
    "shadow-m ring ring-goldfinch-line fixed top-1/2 left-1/2 w-full sm:w-auto max-w-[calc(100vw-2rem)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl bg-goldfinch-base text-goldfinch-default duration-150 data-ending-style:scale-90 data-ending-style:opacity-0 data-starting-style:scale-90 data-starting-style:opacity-0",
    // Apply size from GOLDFINCH_DIALOG_VARIANTS
    resolveVariant(GOLDFINCH_DIALOG_VARIANTS.size, size, GOLDFINCH_DIALOG_DEFAULT_VARIANTS.size).classes,
  );
}
 ...
 * ```
 */
export type DialogProps = GoldfinchDialogVariantsProps & {
...
function DialogRoot(props: DialogRootProps) {
  ...
  const {
    children,
    role = GOLDFINCH_DIALOG_DEFAULT_VARIANTS.role,
    ...rootProps
  } = props;
  ...
  if (role === "alertdialog") {
    return (
      <AlertDialogBase.Trigger
        data-goldfinch-component="Dialog"
...
function DialogClose({ children, ...props }: DialogCloseProps) {
  ...
  return (
    <BaseClose data-goldfinch-component="Dialog" data-goldfinch-part="close" {...props}>
      {children}
    </BaseClose>
  );
}
