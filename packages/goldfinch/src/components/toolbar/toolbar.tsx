...
import { Button as GoldfinchButton, type ButtonProps } from "../button/button";
import { Input as GoldfinchInput, type InputProps } from "../input/input";
import { InputGroup } from "../input-group/input-group";

export const GOLDFINCH_TOOLBAR_VARIANTS = {
  size: {
    xs: {
      classes: "text-xs",
...
export const GOLDFINCH_TOOLBAR_DEFAULT_VARIANTS = {
  size: "base",
} as const;

export type ToolbarSize = keyof typeof GOLDFINCH_TOOLBAR_VARIANTS.size;
...
const ToolbarSizeContext = createContext<{ size: ToolbarSize }>({
  size: GOLDFINCH_TOOLBAR_DEFAULT_VARIANTS.size,
});
...
function toolbarControlClassName(className?: string) {
  return cn(
    ...
    "not-first:border-l not-first:border-goldfinch-line",
    "focus:ring-goldfinch-focus/50 focus:ring-[1.5px] focus-visible:ring-2 focus-visible:ring-goldfinch-brand",
    className,
  );
}
...
const Root = React.forwardRef<HTMLDivElement, ToolbarProps>(
  (
    ...
    return (
      <ToolbarBase.Root
        ref={ref}
        data-goldfinch-component="Toolbar"
        className={cn(
          "inline-flex w-fit items-stretch rounded-lg ring ring-goldfinch-line bg-goldfinch-control shadow-xs",
          GOLDFINCH_TOOLBAR_VARIANTS.size[size].classes,
          className,
        )}
        {...props}
      >
...
const Button = React.forwardRef<HTMLButtonElement, ToolbarButtonProps>(
  (
    ...
    const button =
      resolvedShape === "base" ? (
        <GoldfinchButton
          className={toolbarControlClassName(className as string | undefined)}
          disabled={disabled}
          icon={IconComponent}
          loading={loading}
          shape="base"
    ...
    return (
      <ToolbarBase.Button
        ref={ref}
        data-goldfinch-component="Toolbar.Button"
...
const Input = React.forwardRef<HTMLInputElement, ToolbarInputProps>(
  ({ className, style, ...props }, ref) => {
    ...
    return (
      <ToolbarBase.Input
        ref={ref}
        data-goldfinch-component="Toolbar.Input"
        render={
          <GoldfinchInput
