...
export const GOLDFINCH_MENUBAR_VARIANTS = {
  // MenuBar currently has no variant options but structure is ready for future additions
} as const;

export const GOLDFINCH_MENUBAR_DEFAULT_VARIANTS = {} as const;

// Derived types from GOLDFINCH_MENUBAR_VARIANTS
export interface GoldfinchMenuBarVariantsProps {}

export function menuBarVariants(_props: GoldfinchMenuBarVariantsProps = {}) {
  return cn(
    // Base styles
    "flex rounded-lg border border-goldfinch-recessed bg-goldfinch-recessed pl-px shadow-xs transition-colors",
  );
}
...
const MenuOption = ({
  ...
  const button = (
    <button
      data-goldfinch-component="MenuBar"
      data-goldfinch-part="option"
      aria-label={tooltip}
      className={cn(
        "relative -ml-px flex h-full w-11 cursor-pointer items-center justify-center rounded-md border-none bg-goldfinch-recessed first:rounded-l-lg last:rounded-r-lg transition-colors focus:z-3 focus:outline-none focus:ring-goldfinch-focus/50 focus-visible:z-3 focus-visible:ring-2 focus-visible:ring-goldfinch-brand",
        {
          "z-2 bg-goldfinch-base shadow-xs transition-colors": isActive === id,
        },
      )}
      onClick={onClick}
    >
...
export const MenuBar = ({
  ...
  return (
    <nav
      className={cn(
        "isolate flex rounded-lg ring ring-goldfinch-line bg-goldfinch-recessed pl-px shadow-xs transition-colors",
        className,
      )}
      ref={menuRef}
    >
