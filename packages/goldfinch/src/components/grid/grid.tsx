...
export const GOLDFINCH_GRID_VARIANTS = {
  variant: {
    "2up": {
...
export const GOLDFINCH_GRID_DEFAULT_VARIANTS = {
  gap: "base",
} as const;

export type GoldfinchGridVariant = keyof typeof GOLDFINCH_GRID_VARIANTS.variant;
export type GoldfinchGridGap = keyof typeof GOLDFINCH_GRID_VARIANTS.gap;
...
export function gridVariants({
  variant,
  gap = GOLDFINCH_GRID_DEFAULT_VARIANTS.gap,
}: {
  variant?: GoldfinchGridVariant;
  gap?: GoldfinchGridGap;
} = {}) {
  return cn(
    "grid",
    variant && resolveVariant(GOLDFINCH_GRID_VARIANTS.variant, variant, "2up").classes,
    resolveVariant(GOLDFINCH_GRID_VARIANTS.gap, gap, GOLDFINCH_GRID_DEFAULT_VARIANTS.gap).classes,
  );
}
...
export function gridItemVariants({
  ...
  return cn(
    mobileDivider &&
      variant === "4up" &&
      "border-b border-goldfinch-hairline pb-8 md:border-b-0 md:pb-0",
  );
}
