...
export const GOLDFINCH_LOADER_VARIANTS = {
  size: {
    sm: {
      value: 16,
...
export const GOLDFINCH_LOADER_DEFAULT_VARIANTS = {
  size: "base",
} as const;

// Derived types from GOLDFINCH_LOADER_VARIANTS
export type GoldfinchLoaderSize = keyof typeof GOLDFINCH_LOADER_VARIANTS.size;

export interface GoldfinchLoaderVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: GoldfinchLoaderSize | number;
}

export function loaderVariants({
  size = GOLDFINCH_LOADER_DEFAULT_VARIANTS.size,
}: GoldfinchLoaderVariantsProps = {}): number {
  if (typeof size === "number") return size;
  return resolveVariant(GOLDFINCH_LOADER_VARIANTS.size, size, GOLDFINCH_LOADER_DEFAULT_VARIANTS.size).value;
}
